...
function connectionFailureHandler(self, event) {
  return function(err) {
    if(self.logger.isInfo()) {
      self.logger.info(f("Connection failed for connection in pool (id: %d) - %s", self.id, err))
    }
...
function flushMonitoringOperations(queue, logger, reason) {
  for(var i = 0; i < queue.length; i++) {
    if(queue[i].monitoring) {
      var workItem = queue[i];
      queue.splice(i, 1);
      if(logger.isInfo()) {
        logger.info("no connection available for monitoring - " + reason);
      }
  ...
  return function() {
    ...
    waitForAuth(function() {
      ...
      while(true) {
        ...
        if(self.availableConnections.length == 0) {
          // Flush any monitoring operations
          flushMonitoringOperations(self.queue, self.logger,
            f("No connections available in connection pool (id: %d, max size: %d, queue length: %d, available: %d, connecting: %d, in use: %d)",
              self.id, self.options.size, self.queue.length, self.availableConnections.length, self.connectingConnections.length, self.inUseConnections.length));
          break;
        }
        ...
        if(connection.isConnected()) {
          ...
          flushMonitoringOperations(self.queue, self.logger,
            f("Connection is disconnected (available: %d, connecting: %d, in use: %d)", self.availableConnections.length, self.connectingConnections.length, self.inUseConnections.length));
        }
      }
    });
